...
',
        ...
        'interpolation in single quotes is allowed by default': function (source) {
            const errors = coffeelint.lint(source);
            assert.isArray(errors);
            return assert.isEmpty(errors);
        },

        'interpolation in single quotes can be forbidden': function (source) {
...
`,
        ...
        'interpolation in double quotes is always allowed': function (source) {
            const config = { no_interpolation_in_single_quotes: { level: 'error' } };
            const errors = coffeelint.lint(source, config);
            assert.isArray(errors);
            return assert.isEmpty(errors);
